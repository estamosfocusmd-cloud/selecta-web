...
async function sendVerificationEmail(to, name, token) {
  ...
  `);
  await client.emails.send({ from: FROM, to, subject: 'Verificá tu cuenta en Selecta', html });
}
...
async function sendResetEmail(to, name, token) {
  ...
  `);
  await client.emails.send({ from: FROM, to, subject: 'Recuperá tu contraseña de Selecta', html });
}
